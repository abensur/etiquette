...
void async function () {
  ...
  async function fetchRepositories(flatdeps: string[]) {
    await spinner(
      `Getting the url for ${flatdeps.length} repositor${ flatdeps.length > 1 ? 'ies' : 'y' }.`,
      async () => {
        repositories = await Promise.all(flatdeps.map(async (dep: string) => {
  ...
  function getUniqueRepos(repositories: string[]) {
    return [...new Set(repositories)].filter(Boolean).map((it: any) => it.split('/').slice(-2))
  }

  function echoRepoCount(uniqueRepos: string[]) {
  ...
  async function starRepo(repo: any) {
    let [owner, name] = repo;
    try {
      await spinner(
  ...
  function echoFailedRepos(failed: string[]) {
    if (failed.length) {
      failed.forEach(it => echo`${chalk.red('✗')} - ${chalk.bold(it)}`)
    }
  }

  function handleError(p: any) {
